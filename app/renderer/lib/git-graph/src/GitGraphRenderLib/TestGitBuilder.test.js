...
describe('TestGitBuilder', () => {
  ...
  it('should display a simple branching & merging structure', () => {
    const git = new TestGitBuilder()
    const { sha: branch1Sha } = git.addCommit({ id: 'branch1', parentId: 'root' })
    const { sha: branch2Sha } = git.addCommit({ id: 'branch2', parentId: 'root' })
    ...
    expect(commits[0].parents).not.toEqual([
      branch2Sha,
      branch1Sha,
    ])
  })
